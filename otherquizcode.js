...
const OtherQuizCode = ({ template, setSelectedTemplate }) => {
  ...
  const handleSubmitQuiz = () => {
    if (quizTitle.trim() === '') {
      alert('Please enter the quiz title before submitting.');
      return;
    }
    ...
    const quizData = {
      ...
      title: quizTitle.trim(), // ✅ Changed from quizTitle to title
      questions: questionsList,
    };

    const existingQuizzes = JSON.parse(localStorage.getItem("otherQuizzes")) || [];
    existingQuizzes.push(quizData);
    localStorage.setItem("otherQuizzes", JSON.stringify(existingQuizzes));
  ...
  return (
    <div className="container-fluid p-3 quiz-container">
      <div className="quiz-bg-section p-4 rounded"
        style={{
      ...
      >
        <div className="fw-bold text-white text-center mb-3 template-heading">
          <label className="text-white fw-bold">Enter Your Quiz Title:</label>
          <input
            type="text"
